...
// ── MIGRATIONS-CLEANUP (V4.4.2 Rollback): ─────────────────
// Die frühere V4.4.2-Migration hat ZinsPlus fälschlich in
// Bibliotheks-Einträge injiziert und die Summe überhöht.
// Dieser Code macht das rückgängig, damit ein sauberer
// Neuimport über „Depot laden" die korrekte Summe liefert.
(function rollbackBadLibraryMigration() {
    const ZINSPLUS_KEY = 'block-kasse::ZinsPlus';
    const ZINSPLUS_AMT = 25079.06;
    const LIB_KEY = 'depotBibliothek_V44';
    try {
        ...
        lib.forEach(entry => {
            ...
            // Erkenn: ZinsPlus ist drin und Gesamtsumme ist ~25079 zu hoch
            // (= genau der fälschlich addierte Betrag)
            if (fi[ZINSPLUS_KEY] === ZINSPLUS_AMT && Math.abs(total - 641470.38) < 1) {
                delete fi[ZINSPLUS_KEY];
                ...
                changed = true;
                console.log(`[V4.4.2 Rollback] ${entry.name}: ZinsPlus entfernt → totalInvestment = ${newTotal.toLocaleString('de-DE', {minimumFractionDigits:2})} €`);
            }
        });
        if (changed) localStorage.setItem(LIB_KEY, JSON.stringify(lib));
    } catch (e) {}
})();
