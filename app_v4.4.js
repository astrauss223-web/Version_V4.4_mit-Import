...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function applyImport(matched, mappedUnmatched) {
        ...
        // totalInvestment = nur gematchte Fonds (ohne Empfehlungsliste)
        // → Gesamtvermögen = Einmal Verplant → verbleibend = 0,00
        // Der User kann das Gesamtvermögen danach manuell erhöhen.
        if (portfolioGlobals.totalInvestment === 0 && totalEinmal > 0) {
            portfolioGlobals.totalInvestment = totalEinmal;
            if (totalInvestmentInput) totalInvestmentInput.value = formatNumberInput(totalEinmal);
        }
    ...
    function loadPortfolioFromJson(data) {
        ...
        portfolioGlobals = {
            totalInvestment: data.globals?.totalInvestment || 0,
            totalSparrate:   data.globals?.totalSparrate || 0,
            fundInvestments: data.globals?.fundInvestments || {},
            fundSparrates:   data.globals?.fundSparrates || {}
        };
