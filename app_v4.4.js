...
// ── HARD RESET ─────────────────────────────────────────────
// Löscht ALLE gespeicherten Daten dieser App und lädt die Seite neu.
// Wird über den roten "Hard Reset"-Button im Header ausgelöst.
window.__hardResetApp = function() {
    const keysToDelete = [];
    for (let i = 0; i < localStorage.length; i++) {
        const k = localStorage.key(i);
        if (k && (k.includes('V44') || k.includes('portfolioGlobals') ||
    ...
    console.log('[Hard Reset] Gelöschte Keys:', keysToDelete);
    location.reload();
};
...
document.addEventListener('DOMContentLoaded', () => {
        ...
        hardResetBtn.addEventListener('click', () => {
            ...
            overlay.innerHTML = `
                <div style="background:#1a1a2e; border:2px solid #dc2626; border-radius:14px;
                    ...
                    <div style="color:#fff; font-size:1.15em; font-weight:700; margin-bottom:8px;">
                        Hard Reset
                    </div>
                    <div style="color:#fca5a5; font-size:0.9em; margin-bottom:8px; line-height:1.5;">
                        Alle gespeicherten Daten werden gelöscht:
                    </div>
                    <ul style="color:#e5e7eb; font-size:0.87em; text-align:left; margin:0 0 20px 20px; line-height:1.8;">
                        <li>Aktives Portfolio &amp; Beträge</li>
                        <li>Depot-Bibliothek (alle Mappeneinträge)</li>
                        <li>Delistete &amp; Empfehlungsliste-Daten</li>
                    </ul>
                    <div style="color:#9ca3af; font-size:0.83em; margin-bottom:24px;">
                        Die Seite wird danach neu geladen.<br>
                        <strong style="color:#fca5a5;">Dieser Vorgang kann nicht rückgängig gemacht werden.</strong>
                    </div>
                    <div style="display:flex; gap:12px; justify-content:center;">
                        <button id="hr-cancel" style="padding:10px 24px; border-radius:8px;
                            border:1px solid #4b5563; background:transparent; color:#d1d5db;
                            cursor:pointer; font-size:0.95em;">Abbrechen</button>
                        <button id="hr-confirm" style="padding:10px 28px; border-radius:8px;
                            border:none; background:#dc2626; color:#fff; cursor:pointer;
                            font-size:0.95em; font-weight:700;">Ja, alles löschen</button>
                    </div>
                </div>`;
    ...
    let _searchTargetFundName = null; // Name des gesuchten Fonds für Auto-Scroll im Modal
        ...
        searchInput.addEventListener('input', (e) => {
            ...
            matches.forEach(match => {
                ...
                li.addEventListener('click', () => {
                    ...
                    // Fond-Name für Auto-Scroll nach Modal-Öffnung merken
    ...
    const updateFinancialOverview = () => {
        ...
        managementBlocks.forEach(block => {
            ...
            // Für Allokations-% nur echte Fonds (be), für Verplant-Anzeige alle inkl. Empfehlungsliste
            totalDistributedEinmal   += be + eInv;
            totalDistributedSparrate += bs + eSpar;
        });
        ...
        if (layerBreakdownList) {
            ...
            document.querySelectorAll('.layer-assigned-amount,.layer-assigned-amount-right').forEach(b => { b.classList.remove('visible'); b.innerHTML = ''; });
            document.querySelectorAll('.tower-layer').forEach(l => l.classList.remove('has-assigned-amount'));
                ...
                ids.forEach(blockId => {
                    ...
                    if (layerEl) {
                        ...
                        if (amountEinmal > 0 && amountSpar > 0) {
                            badgeHtml = `<div class="badge-line badge-einmal"><span class="badge-label">Einmal:</span> <span class="badge-val">${formatCurrency(amountEinmal)}</span></div><div class="badge-line badge-sparrate"><span class="badge-label">Sparrate:</span> <span class="badge-val">${formatCurrency(amountSpar)} mtl.</span></div>`;
                        } else if (amountEinmal > 0) {
                            badgeHtml = `<div class="badge-line badge-einmal"><span class="badge-label">Einmal:</span> <span class="badge-val">${formatCurrency(amountEinmal)}</span></div>`;
                        } else if (amountSpar > 0) {
                            badgeHtml = `<div class="badge-line badge-sparrate"><span class="badge-label">Sparrate:</span> <span class="badge-val">${formatCurrency(amountSpar)} mtl.</span></div>`;
                        }

                        badge.innerHTML = badgeHtml;
                        badge.classList.add('visible');
                        layerEl.classList.add('has-assigned-amount');
                    }
                });
            }
        }
    ...
    const updatePortfolioCountrySummary = () => {
        ...
        const summaryContainerSparrate = document.getElementById('portfolio-country-summary-sparrate');
        const clusterListElSparrate   = document.getElementById('portfolio-cluster-list-sparrate');
        ...
        // Aggregiert Länderwerte für Einmalbeitrag oder Sparrate
        const computeClusterData = (isSparrate = false) => {
            let countryValues         = {};
            ...
            portfolio.forEach(layer => {
                layer.funds.forEach(fund => {
                    const key = `${layer.blockId}::${fund.name}`;
                    const val = isSparrate
                        ? (portfolioGlobals.fundSparrates[key] || 0)
                        : (portfolioGlobals.fundInvestments[key] || 0);
            ...
            return { countryValues, countryValuesAktien, countryValuesAnleihen, totalAmount };
        };

        // Rendert Cluster-Pills in die entsprechende Karte
        const renderClusters = (containerEl, listEl, data, cardTitleType) => {
            const { countryValues, countryValuesAktien, countryValuesAnleihen, totalAmount } = data;

            if (totalAmount <= 0 || Object.keys(countryValues).length === 0) {
                listEl.innerHTML = `<div class="empty-breakdown" style="padding:12px 0;color:#888;font-size:0.82rem;text-align:center;">Keine ${cardTitleType} zugewiesen</div>`;
            ...
            CLUSTER_ORDER.forEach(clusterKey => {
                ...
                pill.innerHTML = `
                    ...
                    <div class="cluster-popup">
                        <div style="font-size:0.82em;font-weight:600;border-bottom:1px solid rgba(255,255,255,0.2);padding-bottom:5px;margin-bottom:7px;color:${cluster.color}">${clusterLabel} \u2013 ${pct.toFixed(1).replace('.', ',')}% (${cardTitleType})</div>
        ...
        const dataEinmal   = computeClusterData(false);
        const dataSparrate = computeClusterData(true);

        renderClusters(summaryContainer, clusterListEl, dataEinmal, 'Einmalbeitrag');
        if (summaryContainerSparrate && clusterListElSparrate) {
            renderClusters(summaryContainerSparrate, clusterListElSparrate, dataSparrate, 'Sparrate');
        }
    };
    ...
    reversedBlocks.forEach((block, index) => {
        ...
        blockEl.addEventListener('click', () => {
            ...
            // ── Such-Scroll: Nach Öffnung zum gesuchten Fonds springen ──
            if (_searchTargetFundName) {
                const targetName = _searchTargetFundName;
                _searchTargetFundName = null; // Reset
                setTimeout(() => {
                    const allItems = modalList.querySelectorAll('.fund-list-item');
                    let targetItem = null;
                    allItems.forEach(item => {
                        if (item.dataset.fundName === targetName) targetItem = item;
                    });
                    if (targetItem) {
                        targetItem.scrollIntoView({ behavior: 'smooth', block: 'center' });
                        targetItem.classList.add('search-highlight-flash');
                        setTimeout(() => targetItem.classList.remove('search-highlight-flash'), 2500);
                    }
                }, 200);
            }
        });
    ...
    // ── Fonds-Zuordnung aus fund_data.js (erfasst alle gelisteten & delisteten Fonds) ──
    function getKnownBlockForFund(wkn, name) {
        const normWkn = wkn ? normalizeWKN(wkn) : '';
        if (normWkn.length === 6) {
            // Zuerst in allen Hauptblöcken (außer Tagesgeld) suchen
            for (const block of managementBlocks) {
                if (block.id === 'block-tagesgeld') continue;
                for (const fund of block.funds) {
                    if (fund._isEmpfehlungsliste) continue;
                    const matches = fund.info ? fund.info.matchAll(/WKN:\s*([A-Z0-9]{6})/gi) : [];
                    for (const m of matches) {
                        if (normalizeWKN(m[1]) === normWkn) {
                            return block.id; // Exakter Block aus fund_data.js
                        }
                    }
                }
            }
            // Fallback: Tagesgeld prüfen falls nur dort vorhanden
            const tgBlock = managementBlocks.find(b => b.id === 'block-tagesgeld');
            if (tgBlock) {
                for (const fund of tgBlock.funds) {
                    if (fund._isEmpfehlungsliste) continue;
                    const matches = fund.info ? fund.info.matchAll(/WKN:\s*([A-Z0-9]{6})/gi) : [];
                    for (const m of matches) {
                        if (normalizeWKN(m[1]) === normWkn) return tgBlock.id;
                    }
                }
            }
        }
        if (name) {
            ...
            for (const block of managementBlocks) {
                for (const fund of block.funds) {
                    if (fund._isEmpfehlungsliste) continue;
                    if (fund.name && fund.name.toLowerCase().trim() === normName) {
                        return block.id;
                    }
                }
            }
    ...
    function anlageschwerpunktToBlock(schwerpunkt) {
        ...
        // 1. Vermögensverwalter-Kategorien zuerst prüfen (verhindert Fehlzuordnung durch "anleihen")
        ...
        // 2. Kasse / Geldmarkt / Kurzläufer
        if (s.includes('geldmarkt')) return 'block-kasse';
        if (s.includes('anleihen euro kurz') || s.includes('anleihen euro kurz laufzeit') || s.includes('anleihen hochzins laufzeit')) return 'block-kasse';

        // 3. Spezialitäten / Alternative Strategien
        if (s.includes('alternative volatilitätsstrategien') || s.includes('alternative')) return 'block-spezial';

        // 4. Gemischte Anlagestruktur (Aktien/Anleihen)
        if (s.includes('aktien/anleihen') || s.includes('anleihen/aktien')) return 'block-ausgewogen';

        // 5. Reines Anleihen / Renten / Bonds
    ...
    function saveLibraryEntry(name, sourceFile, data) {
        ...
        // totalValue immer aus fundInvestments neu berechnen – gespeicherter Wert kann falsch sein
        const recalcTotal = Object.values(data.globals?.fundInvestments || {}).reduce((s, v) => s + v, 0);
        const entry = {
            name, sourceFile,
            savedAt: new Date().toISOString(),
            fundCount: Object.keys(data.globals.fundInvestments || {}).length,
            totalValue: recalcTotal > 0 ? recalcTotal : (data.globals.totalInvestment || 0),
            data
        };
    ...
    function buildWknLookup() {
        ...
        // behält nur der Block mit der HÖCHSTEN Priorität seinen Eintrag.
        // Prioritätsreihenfolge = Reihenfolge von managementBlocks (erster = höchste Priorität),
        // block-tagesgeld hat immer die geringste Priorität.
        const blockOrder = managementBlocks.map(b => b.id);
        Object.keys(map).forEach(wkn => {
            if (map[wkn].length <= 1) return;
            // block-tagesgeld zuerst herausfiltern
            ...
            // Unter den verbleibenden: Block mit kleinstem Index in blockOrder behalten
    ...
    function parseCsvDepot(text, wknLookup) {
        ...
        for (let i = 0; i < clean.length; i++) {
            ...
            // Sonderfall: Betrag steht auf der Zeile ÜBER der WKN.
            // Format: ";;Schwerpunkt;;Betrag;" (vorige Zeile, keine WKN)
            //         "A0MUWS;;;;;"            (aktuelle Zeile, kein Betrag)
    ...
    function parseXlsxDepot(workbook, wknLookup) {
        ...
        // Betrag aus einer Row lesen (Spalte 4 = Kurswert, Fallback Spalte 3)
        // Bereinigt non-breaking spaces (\u00a0) und €-Zeichen
        function getAmt(row) {
            for (const idx of [4, 3]) {
                const raw = String(row[idx] === null || row[idx] === undefined ? '' : row[idx])
        ...
        for (let i = 0; i < rows.length; i++) {
            const row = rows[i];
            // WKN kann Integer (z.B. 847809) oder String ('A0MUWS') sein
            const rawCell = row[0];
            const wkn = normalizeWKN(
                String(rawCell === null || rawCell === undefined ? '' : rawCell).trim()
            );
            if (wkn.length !== 6) continue;
            ...
            // Sonderfall: Betrag steht auf der Zeile ÜBER der WKN (wie bei A0MUWS).
            // Erkennbar daran: WKN-Zeile hat keinen Betrag,
            // aber Vorherzeile hat einen Betrag und keine gültige WKN.
            if (einmal <= 0 && i > 0) {
                const prev = rows[i - 1];
                const prevWkn = normalizeWKN(
                    String(prev[0] === null || prev[0] === undefined ? '' : prev[0]).trim()
                );
                if (prevWkn.length !== 6) {
                    einmal = getAmt(prev);
            ...
            const nameRaw = i + 1 < rows.length
                ? String(rows[i + 1][0] === null || rows[i + 1][0] === undefined ? '' : rows[i + 1][0]).trim()
                : '';
    ...
    function applyImport(matched, mappedUnmatched) {
        // ← Zuerst ALLES löschen: keine alten Werte dürfen die neue Summe beeinflussen
        ...
        // totalInvestment = IMMER als Summe aller fundInvestments berechnen
        // (= gematchte Fonds + Empfehlungsliste-Fonds), damit kein Betrag verloren geht.
    ...
    function classifyUnmatched(unmatched) {
        ...
        unmatched.forEach(u => {
            // 1. Zuerst in fund_data.js prüfen (deckt alle bekannten delisteten & aktiven Fonds ab)
            let blockId = getKnownBlockForFund(u.wkn, u.name);

            // 2. Falls WKN/Name nicht in fund_data.js hinterlegt ist, über Anlageschwerpunkt bestimmen
            if (!blockId) {
                blockId = anlageschwerpunktToBlock(u.schwerpunkt);
            }

            if (blockId) {
                const knownName = getKnownFundName(u.wkn) || u.name;
                mapped.push({ ...u, name: knownName, blockId });
            } else {
                ambiguous.push(u);
            }
        });
    ...
    function buildImportJson(matched, unmatched) {
        const { mapped } = classifyUnmatched(unmatched);
        const fundInvestments = {};
        const fundSparrates   = {};
        matched.forEach(({ block, fund, einmal, sparrate }) => {
            const key = `${block.id}::${fund.name}`;
            if (einmal   > 0) fundInvestments[key] = einmal;
            if (sparrate > 0) fundSparrates[key]   = sparrate;
        });
        // Empfehlungsliste-Fonds ebenfalls in fundInvestments aufnehmen
        mapped.forEach(u => {
            const eKey = `${u.blockId}::empfehlungsliste`;
            fundInvestments[eKey] = (fundInvestments[eKey] || 0) + (u.einmal || 0);
            if (u.sparrate > 0) fundSparrates[eKey] = (fundSparrates[eKey] || 0) + u.sparrate;
        });
        const globals = {
            totalInvestment: Object.values(fundInvestments).reduce((s, v) => s + v, 0),
