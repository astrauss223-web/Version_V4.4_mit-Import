...
// ── MIGRATIONS-FIX (V4.4.2): Bibliotheks-Einträge korrigieren ─
// Der alte CSV-Parser hat A0MUWS (ZinsPlus) übersprungen, weil
// Betrag und WKN in getrennten Zeilen standen. Dieser Fix injiziert
// den fehlenden Betrag in alle betroffenen Bibliotheks-Einträge.
(function fixLibraryEntries() {
    const ZINSPLUS_KEY = 'block-kasse::ZinsPlus';
    const ZINSPLUS_AMT = 25079.06; // Betrag aus Mappe1.csv
    const LIBRARY_KEY_MFIX = 'depotBibliothek_V44';
    try {
        const raw = localStorage.getItem(LIBRARY_KEY_MFIX);
        if (!raw) return;
        const lib = JSON.parse(raw);
        let changed = false;
        lib.forEach(entry => {
            if (!entry.data || !entry.data.globals) return;
            const fi = entry.data.globals.fundInvestments || {};
            // Nur korrigieren wenn ZinsPlus fehlt (= war unmatched wegen Bug)
            // UND Basis-Fonds I Nachhaltig vorhanden ist (= das ist das korrekte Depot)
            if (!fi[ZINSPLUS_KEY] && fi['block-kasse::Basis-Fonds I Nachhaltig']) {
                fi[ZINSPLUS_KEY] = ZINSPLUS_AMT;
                entry.data.globals.fundInvestments = fi;
                // totalInvestment neu berechnen
                const newTotal = Object.values(fi).reduce((s, v) => s + v, 0);
                entry.data.globals.totalInvestment = newTotal;
                entry.totalValue = newTotal;
                changed = true;
                console.log(`[V4.4.2 Migration] ${entry.name}: ZinsPlus (${ZINSPLUS_AMT.toLocaleString('de-DE')} €) ergänzt → totalInvestment = ${newTotal.toLocaleString('de-DE', {minimumFractionDigits:2})} €`);
            }
        });
        if (changed) localStorage.setItem(LIBRARY_KEY_MFIX, JSON.stringify(lib));
    } catch (e) { console.warn('V4.4.2 Migration fehlgeschlagen:', e); }
})();
